...
class RSC {
	// placeholders
	static $cloud = null;
	static $objstore = null;
	static $container = null;

	/**
	 * initialize a connection to Rackspace Files (Setup + ObjectStore)
	 * @return object $objstore
	 */
	public static function init() {
		global $config;
		if (!empty(RSC::$objstore)) {
			return RSC::$objstore;
		}
		require_once CKFINDER_CONNECTOR_LIB_DIR.'/php-opencloud-lib/rackspace.php';
		$cloud = new OpenCloud\Rackspace(RACKSPACE_US, array(
			'username' => $config['Rackspace']['username'],
			'apiKey' => $config['Rackspace']['apiKey'],
			'tenantName' => $config['Rackspace']['account'],
		));
		if (empty($cloud) || !is_object($cloud)) {
			throw new OutOfBoundsException('rscfiles_con() unable to initialize the $cloud connection to Rackspace');
			die();
		}
		require_once CKFINDER_CONNECTOR_LIB_DIR.'/php-opencloud-lib/objectstore.php';
		$objstore = $cloud->ObjectStore('cloudFiles', $config['Rackspace']['DataCenter']);
		if (empty($objstore) || !is_object($objstore)) {
			throw new OutOfBoundsException('rscfiles_con() unable to initialize the $objstore connection to Rackspace Files');
			die();
		}
		RSC::$objstore =& $objstore;
		return $objstore;
	}

	/**
	 * quick access method for finding getting the Container
	 * cached locally once succesfully gathered
	 */
	public static function container($objstore = null) {
		global $config;
		if (!empty(RSC::$container)) {
			return RSC::$container;
		}
		if (empty($objstore)) {
			$objstore = RSC::init();
		}
		...
		while($cdncontainer = $cdnlist->Next()) {
			if ($cdncontainer->name == $config['Rackspace']['Container']) {
				// found container
				$objcontainer = $objstore->Container($cdncontainer->name);
				RSC::$container = $objcontainer;
				return $objcontainer;
			}
		}
		// did not find container
		throw new OutOfBoundsException('Rackspace Files Container not found'); die();
		return false;
	}
}
