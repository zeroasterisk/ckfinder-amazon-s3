...
class CKFinder_Connector_CommandHandler_Thumbnail extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public static function createThumb($sourceFile, $targetFile, $maxWidth, $maxHeight, $quality, $preserverAspectRatio, $bmpSupported = false)
    {
        ...
        {
            case 'image/gif':
                imagegif($oThumbImage, $ftp_con->full_tmp_file);
                break;
            case 'image/jpeg':
            case 'image/bmp':
                imagejpeg($oThumbImage, $ftp_con->full_tmp_file, $quality);
                break;
            case 'image/png':
                imagepng($oThumbImage, $ftp_con->full_tmp_file);
                break;
            case 'image/wbmp':
                imagewbmp($oThumbImage, $ftp_con->full_tmp_file);
                break;
        }
		
		$ftp_con->move_file($ftp_con->full_tmp_file, $targetFile);

        $_config =& CKFinder_Connector_Core_Factory::getInstance("Core_Config");
        if (file_exists($targetFile) && ($perms = $_config->getChmodFiles())) {
           $ftp_con->chmod($targetFile, $perms);
        }
